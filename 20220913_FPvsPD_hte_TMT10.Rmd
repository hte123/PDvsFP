---
title: "20220913_FPvsPD_hte_TMT10"
author: "Tianen He"
date: "2022/9/13"
...
```{r}
pd_protein = read.csv("./TMT10_PD3/F20200302gaohh_PCT2G_HCC_TMT10_chimerys_intensity_nocontam_hte220912_Proteins.txt", sep = "\t")
pd_peptide = read.csv("./TMT10_PD3/F20200302gaohh_PCT2G_HCC_TMT10_chimerys_intensity_nocontam_hte220912_PeptideGroups.txt", sep = "\t")
```

## FP with contaminants

```{r}
fp_protein_tmt = read.csv("./TMT10_FP18/tmt-report/abundance_protein_None.tsv", sep = "\t")
fp_peptide_tmt = read.csv("./TMT10_FP18/tmt-report/abundance_peptide_None.tsv", sep = "\t")
```
...
```{r}
# colnames(pd_protein)
pd_protein_abundance = pd_protein[,30:39] # 10 samples
colnames(pd_protein_abundance)
```

### Proteins with an abundance of NA in all 10 channels are considered as unquantified in PD

```{r}
# proteins with 10 NA are removed
...
```{r}
# colnames(pd_peptide)
pd_peptide_abundance = pd_peptide[,16:25]
...
```{r}
# proteins with 10 NAs are removed
...
fp_protein_abundance_tmt_real = fp_protein_tmt_real[, 6:15] 
...
```{r}
colnames(fp_protein_abundance_tmt_real)
colnames(fp_protein_abundance_tmt_real) = colnames(fp_protein_abundance_tmt_real) %>% 
  str_replace(., "X", "") %>% str_replace(., "131N", "131")
colnames(fp_protein_abundance_tmt_real)
...
# write.csv(pd_sorted, file = paste0(root, "PD_abundance[log2]_tmt10.csv"), quote = FALSE)
# write.csv(fp_sorted, file = paste0(root, "FP_abundance[log2]_tmt10.csv"), quote = FALSE)
```
...
ggsave(venn, path = root, filename = "FigureS2A_protein_overlap_tmt10.pdf", units = "px",
...
ggsave(file = paste0(root, "FigureS2C_protein_overlap_compare_batch_violin_PDandFP_tmt10.pdf"), 
       plot = p_compare_batch_violin, width = 1600, height = 1000, units = "px")
```

# Correlation coefficients

## correlation of log2 abundances of each sample (10 channels)
...
pheatmap(cor_matrix, 
         ...
         filename = paste0(root, "protein_cor_matrix_tmt10.pdf"), 
         # main = "Pearson Correlation Matrix",
         color = cividis(100, direction = 1),
         width = 4.5, height = 4, fontsize = 8,
...
ggsave(plot = hist_plot, 
       file = paste0(root, "FigureS2E_correlation_coefficient_distribution_tmt10_samples.pdf"),
...
hist_plot_t = ggplot(data.frame(cor_self_t), aes(x = cor_self_t, y = ..count..)) +
    geom_histogram(bins = 30, fill = colors[10], color = "white") +
    # ggtitle("Distribution of Pearson Correlation Coefficient") + 
    xlab("Spearman's correlation coefficient") +
...
ggsave(plot = hist_plot_t, file = paste0(
    root, "FigureS2D_correlation_coefficient_distribution_tmt10_proteins.pdf"),
       width = 1200, height = 1000, units = "px")
hist_plot_t
```
